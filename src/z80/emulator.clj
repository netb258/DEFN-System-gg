    ...
    (IORead [this address]
      (let [port (signed->unsigned address)
        ...
        (cond
          ...
          (= port-group 0x00)
          (if (even? port)
            0xFF ;; Port $00 is unmapped/open bus
          ...
          (= port-group 0x80)
          (if (even? port)
            (do-vdp-io-read! active-vdp vdp/data-read!) ;; VDP DATA PORT ($BE)
            (do-vdp-io-read! active-vdp vdp/read-status-port! cpu)) ;; VDP STATUS PORT ($BF)

          ;; --- Group 0xC0 to 0xFF ---
          (= port-group 0xC0)
          (if (even? port)
            @joypad-p1 ;; Even ports ($DC) = P1 Input
            ;; Odd ports ($DD) = P2 Input + Region/Video Bits
            (let [joy2 @joypad-p2]
              (cond
                ;; Game expects Japanese/NTSC response when it writes 0x55
                (= @port-3f-latch 0x55)
                (bit-and joy2 0x3F)   ;; Force Bits 6 and 7 to 0

                ;; Game expects Export/PAL response when it writes 0xF5
                (= @port-3f-latch 0xF5)
                (bit-or joy2 0xC0)    ;; Force Bits 6 and 7 to 1

                ;; Default fallback (Standard Export / English mode)
                :else (bit-or joy2 0xC0))))
    ...
    (IOWrite [this address data]
      (let [port (signed->unsigned address)
        ...
        (cond
          ...
          ;; This catches the game trying to test the console type
          (and (= port-group 0x00) (= port 0x3F))
          (reset! port-3f-latch data)
...
(defn draw-all-sprites-line-for-scanline!
  ...
  (let [vdp                 @active-vdp
    ...
    (let [reg0 (aget vdp-regs 0) ;; Read Register 0 to check for the Early Clock (EC) Sprite Shift flag (Bit 3)
      ...
      ;; NOTE: This used to be (take 8 matching-sprites), but why have the original flickering sprites.
      ...
      (when (> (count matching-sprites) 8)
        (swap! active-vdp assoc :sprite-overflow? true)))
...
(defn setup []
  ...
  ;; NOTE: Actually let's switch it up to 60.
  (q/frame-rate 60)
  (reset! global-frame-buffer (q/create-image 256 224 :rgb))
...
(defn draw-scanlines! [screen-width screen-height opacity]
  ...
  (q/stroke-weight 2)      ; 2 pixel thick lines
  (loop [y 0]
...
(defn draw []
  ...
  (let [frame-canvas @global-frame-buffer
        ;; Keep full width (including left overscan) but drop the bottom 32 rows
        ;; X: starts at 0, width is 256
        ;; Y: starts at 0, height is 192 (224 - 32)
        sms-viewport (.get ^processing.core.PImage frame-canvas 0 0 256 192)]

    ;; Paint the cropped view scaled up to your target screen dimensions
    (q/image sms-viewport 0 0 screen-width screen-height))

  ;; 4. Draw the scanline overlay (Opacity set to 40 out of 255 for a subtle effect)
  ;; (draw-scanlines! screen-width screen-height 40)
  ;; NOTE: Let's try some OpenGL shaders:
  (q/shader (q/load-shader "./shaders/crt-light.glsl"))
